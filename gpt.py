...
def gpt_ensemble(
    ...
    for sent in tqdm(candidate_json):
        ...
        for group in sent["merged_entities"]:
            ...
            for i in range(iter):
                final_text, final_type, explanation = run_gpt(gpt_api, prompt, query, gpt_ver, gpt_temperature)
...
def get_arguments():
    ...
    parser.add_arguemnt("--output_dir")
    parser.add_arguemnt("--api_key")
    parser.add_arguemnt("--version", default="gpt-3.5-turbo-0125")
    parser.add_argument("--temperature", default=0.7)
    parser.add_argument("--num_gpt_calls", default=3)
    parser.add_arugment("--umls_version", defualt="2023AA")
    ...
    parser.add_arguemnt("--overwrite_gpt_output", action="store_true")
    parser.add_arugment("--type_norm_file", default=None)
...
if __name__ == "__main__":
    ...
    umls_api = UMLS_API(api_key=args.umls_api_key, version=args.version,
    ...
    ensemble_path = os.path.join(args.output_dir, "ensemble.json")

    if os.path.exists(ensemble_path) and args.overwrite_gpt_output:
        print(f"The ensemble file ({ensemble_path}) already exists. To reprocess and overwrite it, "
            f"please use the --overwrite_gpt_output argument.")
        exit()

    if not os.path.exists(candidate_path) or args.overwrite_preprocessed_file:
        for i, iob_path in enumerate(args.input_list.readlines()):
            iob_path = iob_path.strip()
            json_path = os.path.join(args.output_dir, f"model{i}.json")
            ...
            json_list.append([f"model{i}", json_result])
    ...
    gpt_ensemble(candidate_json, ensemble_path, iter=args.num_gpt_calls, gpt_ver=args.version, gpt_temperature=args.temperture, )
